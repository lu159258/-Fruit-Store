...
public class ItemOrderController extends BaseController {
    ...
    public String my(Model model, HttpServletRequest request){
        ...
        //待发货
        String sql1 = "select * from item_order where user_id="+userId+" and status=0 order by id desc";
        List<ItemOrder> dfh = itemOrderService.listBySqlReturnEntity(sql1);
        //已取消
        String sql2 = "select * from item_order where user_id="+userId+" and status=1 order by id desc";
        List<ItemOrder> yqx = itemOrderService.listBySqlReturnEntity(sql2);
        //已发货
        String sql3 = "select * from item_order where user_id="+userId+" and status=2 order by id desc";
        List<ItemOrder> dsh = itemOrderService.listBySqlReturnEntity(sql3);
        //已收货
        String sql4 = "select * from item_order where user_id="+userId+" and status=3 order by id desc";
     ...
     */
    @RequestMapping("/qx")
    public String qx(Model model,int id){
     ...
     */
    @RequestMapping("/fh")
    public String fh(int id,Model model){
        ItemOrder obj = itemOrderService.load(id);
        obj.setStatus(2);
        itemOrderService.updateById(obj);
        model.addAttribute("obj",obj);
